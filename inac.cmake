...
function(inac_add_tests)
    ...
    file(GLOB src ${CMAKE_SOURCE_DIR}/tests/test_*.c ${CMAKE_SOURCE_DIR}/tests/helper_*.c src ${CMAKE_SOURCE_DIR}/tests/test_*.cxx ${CMAKE_SOURCE_DIR}/tests/helper_*.cxx src ${CMAKE_SOURCE_DIR}/tests/test_*.cpp ${CMAKE_SOURCE_DIR}/tests/helper_*.cpp)
...
function(inac_add_benchmarks)
    ...
    remove_definitions(-DINA_LIB)
    file(GLOB src ${CMAKE_SOURCE_DIR}/bench/bench_*.c ${CMAKE_SOURCE_DIR}/bench/bench_*.cxx ${CMAKE_SOURCE_DIR}/bench/bench_*.cpp)
    list(LENGTH src src_count)
    if (${src_count} EQUAL 0)
...
function(inac_add_tools)
    remove_definitions(-DINA_LIB)
    set(tools "")
    file(GLOB src ${CMAKE_SOURCE_DIR}/tools/*.c ${CMAKE_SOURCE_DIR}/tools/*.cxx ${CMAKE_SOURCE_DIR}/tools/*.cpp)
...
function(inac_add_examples)
    remove_definitions(-DINA_LIB)
    set(examples "")
    file(GLOB src ${CMAKE_SOURCE_DIR}/examples/*.c ${CMAKE_SOURCE_DIR}/examples/*.cxx ${CMAKE_SOURCE_DIR}/examples/*.cpp)
	...
	if (MSVC)
		if(${CMAKE_BUILD_TYPE} MATCHES "Debug")
			if(EXISTS "${INAC_REPOSITORY_PATH}/${DEPENDENCY_NAME}/bin/luajitd.exe")
				set(INAC_LUAJIT_BINARY "${INAC_REPOSITORY_PATH}/${DEPENDENCY_NAME}/bin/luajitd.exe")
			endif()
		else()
			if(EXISTS "${INAC_REPOSITORY_PATH}/${DEPENDENCY_NAME}/bin/luajit.exe")
				set(INAC_LUAJIT_BINARY "${INAC_REPOSITORY_PATH}/${DEPENDENCY_NAME}/bin/luajit.exe")
			endif()
		endif()
	else()
		if(EXISTS "${INAC_REPOSITORY_PATH}/${DEPENDENCY_NAME}/bin/luajit")
				set(INAC_LUAJIT_BINARY "${INAC_REPOSITORY_PATH}/${DEPENDENCY_NAME}/bin/luajit")
			endif()
	endif()
...
function(inac_artifact_name_only name output_var)
    if (MSVC)
        if ($ENV{VisualStudioVersion} STREQUAL "12.0")
            string(APPEND ARTIFACT_NAME "${name}-${CMAKE_SYSTEM_NAME}_vs13-${INAC_TARGET_ARCH}-${CMAKE_BUILD_TYPE}")
        elseif($ENV{VisualStudioVersion} STREQUAL "14.0")
        ...
        string(APPEND ARTIFACT_NAME "${name}-${CMAKE_SYSTEM_NAME}-${INAC_TARGET_ARCH}-${CMAKE_BUILD_TYPE}")
    endif()
    string(TOLOWER ${ARTIFACT_NAME} ARTIFACT_NAME)
    set("${output_var}" ${ARTIFACT_NAME} PARENT_SCOPE)
endfunction()
